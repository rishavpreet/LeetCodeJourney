...
permutation of base[n], is n = 3. However, base[3] has four elements but array nums has three. Therefore, it can not
...
permutation of base[n], is n = 1. It can be seen that nums is a permutation of base[1] = [1, 1].
...
Output: false
Explanation: Since the maximum element of the array is 4, the only candidate n for which this array could be
a permutation of base[n], is n = 4. However, base[4] has five elements but array nums has six. Therefore, it can not
...
class SolutionOne:
    def isGood(self, nums: List[int]) -> bool:
        ...
        if dup_num:
            if min(dup_num) < max_num:
                return False

        if len(set(nums)) + 1 == base and max_num < base:
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    nums = [3, 4, 4, 1, 2, 1]
    output = False
    assert output == sol.isGood(nums=nums)
